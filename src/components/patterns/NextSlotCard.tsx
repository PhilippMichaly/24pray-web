...
export interface NextSlotCardProps {
  ...
  now?: number;
  /** Tages-Wache (slotDurationMinutes=1440): kein Stunden-Bereich, kein „Uhr"-Suffix. */
  dayMode?: boolean;
}

export function NextSlotCard({ slot, projectTimezone, now = Date.now(), dayMode }: NextSlotCardProps) {
    ...
    <Card elevation={1} className="border-accent/30 bg-accent-soft/40">
      <div className="flex items-center gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent/15 text-accent">
          <Flame size={20} className={soon && !dayMode ? 'animate-breathe' : ''} aria-hidden />
        </span>
        <div className="min-w-0">
          <p className="text-xs font-medium uppercase tracking-wide text-accent-strong">
            {tUnit(!!dayMode, 'nextSlotTitle', 'nextSlotTitleDay')}
          </p>
